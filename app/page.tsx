...
function BankSketch() {
  return (
    <svg viewBox="0 0 260 190" className="h-full w-full text-[#3f9e74] opacity-80">
...
export default function Home() {
  ...
  const gridTone = (change: number | null) => change == null || change === 0 ? "text-[#9ba3a6]/60" : change > 0 ? "text-emerald-400" : "text-red-400";
  ...
  return (
    <main className="min-h-screen bg-[#0a0b0b] text-[#edf1f2]">
      <Navbar />
      <div className="pt-16">
        <MarketTickerStrip />
      </div>

      <section className="relative border-b border-[rgba(63,158,116,0.28)] pt-12 lg:pt-16">
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-12 px-6 pb-20 lg:grid-cols-[1fr_420px] lg:px-12">
          <div className="relative flex gap-8">
            <div className="hidden pt-4 lg:block">
              <div className="vertical-rl text-[10px] tracking-[0.35em] text-[#3f9e74]/60 [writing-mode:vertical-rl]">FOCUS. DISCIPLINE. CONSISTENT.</div>
            </div>
            <div className="max-w-3xl pt-6">
              <div className="mb-8 h-px w-24 bg-[#3f9e74]" />
              <h1 className="font-heading text-[clamp(3.4rem,7vw,7.5rem)] font-medium leading-[0.92] tracking-[-0.04em] text-[#edf1f2]">
                Mana saham yang murah beneran?
              </h1>
              <p className="mt-8 max-w-2xl text-base leading-8 text-[#9ba3a6] md:text-lg">
                Catatan riset pribadi tentang valuasi, arus asing, corporate action, dan narasi pasar modal Indonesia.
              </p>
              <div className="mt-10 flex flex-wrap items-center gap-6">
                <a href="/articles" className="border border-[#3f9e74] bg-[#3f9e74] px-6 py-3 text-[11px] font-semibold tracking-[0.18em] text-[#0a0b0b] transition hover:bg-transparent hover:text-[#3f9e74]">
                  BACA RISET TERBARU
                </a>
                <a href="/playbook/ihsg" className="text-[11px] font-semibold tracking-[0.18em] text-[#3f9e74] hover:text-[#edf1f2]">
                  LIHAT MARKET MAP →
                </a>
              </div>
            </div>
          ...
          <aside className="border border-[rgba(63,158,116,0.28)] bg-[#121414]/80 p-6 shadow-2xl shadow-black/30">
            <div className="mb-6 flex items-end justify-between border-b border-[rgba(63,158,116,0.28)] pb-4">
              <div>
                <p className="text-[10px] font-semibold tracking-[0.25em] text-[#3f9e74]">KILASAN PASAR</p>
                <p className="mt-2 text-xs text-[#9ba3a6]/50">{updatedAt || "Memuat data pasar..."}</p>
              </div>
              <div className="h-2 w-2 rounded-full bg-emerald-400" />
            </div>
            <div className="space-y-5">
              {snapshots.map((s) => (
                <div key={s.name} className="grid grid-cols-[1fr_80px_90px] items-center gap-4 border-b border-[rgba(63,158,116,0.28)]/60 pb-4 last:border-0">
                  <div>
                    <p className="text-sm font-semibold text-[#edf1f2]">{s.name}</p>
                    <p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-[#9ba3a6]/45">{s.sub}</p>
                  </div>
                  <svg viewBox="0 0 80 36" className="h-9 w-20">
                    <path d={s.path} fill="none" stroke={s.tone === "down" ? "#d45b4f" : gold} strokeWidth="1.6" />
                  </svg>
                  <div className="text-right">
                    <p className="font-mono text-sm text-[#edf1f2]">{s.value}</p>
                    <p className={`mt-1 font-mono text-xs ${s.tone === "down" ? "text-red-400" : s.tone === "up" ? "text-emerald-400" : "text-[#9ba3a6]/60"}`}>{s.change}</p>
                  </div>
                </div>
              ))}
            </div>
              <div className="mt-3 text-[10px] text-[#9ba3a6]/45">
                {marketStatus === "error" ? "Data pasar tidak tersedia saat ini." : "Sumber: TradingView. Data dapat tertunda."}
              </div>
              <a href="/playbook/ihsg" className="mt-6 inline-block text-[11px] font-semibold tracking-[0.18em] text-[#3f9e74]">LIHAT SELENGKAPNYA →</a>
          </aside>
        </div>
      </section>

      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-10 border-b border-[rgba(63,158,116,0.28)] px-6 py-16 lg:grid-cols-[1fr_360px] lg:px-12">
        <div>
          <div className="mb-8 flex items-center justify-between">
            <h2 className="font-heading text-3xl text-[#edf1f2]">RISET TERBARU</h2>
            <a href="/articles" className="text-[11px] font-semibold tracking-[0.18em] text-[#3f9e74]">LIHAT SEMUA →</a>
          </div>
          <div className="divide-y divide-[rgba(63,158,116,0.28)] border-y border-[rgba(63,158,116,0.28)]">
            {articles.map((a) => (
              <a key={a.title} href={a.href} className="grid gap-5 py-8 transition hover:bg-[#3f9e74]/[0.03] md:grid-cols-[150px_1fr_90px]">
                <p className="text-[11px] tracking-[0.18em] text-[#9ba3a6]/45">{a.date}</p>
                <div>
                  <p className="text-[10px] font-semibold tracking-[0.22em] text-[#3f9e74]">{a.category}</p>
                  <h3 className="mt-3 font-heading text-2xl leading-tight text-[#edf1f2]">{a.title}</h3>
                  <p className="mt-3 max-w-2xl text-sm leading-7 text-[#9ba3a6]/70">{a.summary}</p>
                </div>
                <p className="text-[10px] tracking-[0.18em] text-[#9ba3a6]/40 md:text-right">{a.read}</p>
              </a>
            ))}
          </div>
        </div>
        <div className="flex flex-col justify-between border border-[rgba(63,158,116,0.28)] bg-[#121414] p-8">
          <div className="font-heading text-7xl leading-none text-[#3f9e74]">“</div>
          <blockquote className="mt-4 font-heading text-3xl leading-tight text-[#edf1f2]">
Jangan takut mengakui kesalahan,
            Itu bukan kekalahan,
            Tapi kebijaksanaan.
          </blockquote>
          <div className="mt-10">
            <p className="text-sm text-[#9ba3a6]">— Raga</p>
            <a href="/#about" className="mt-6 inline-block text-[11px] font-semibold tracking-[0.18em] text-[#3f9e74]">TENTANG RAGA →</a>
          </div>
        </div>
      </section>

      <section id="about" className="mx-auto max-w-7xl border-b border-[rgba(63,158,116,0.28)] px-6 py-16 lg:px-12">
        <h2 className="mb-10 font-heading text-4xl text-[#edf1f2]">CARA MEMBACA MARKET</h2>
        <div className="grid gap-px bg-[rgba(63,158,116,0.28)] md:grid-cols-2 lg:grid-cols-4">
          {method.map((m) => (
            <div key={m.title} className="bg-[#0a0b0b] p-7">
              <Icon type={m.icon} />
              <h3 className="mt-7 font-heading text-2xl text-[#edf1f2]">{m.title}</h3>
              <p className="mt-4 text-sm leading-7 text-[#9ba3a6]/70">{m.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-[1fr_460px] lg:px-12">
        <div className="grid overflow-hidden border border-[rgba(63,158,116,0.28)] bg-[#121414] md:grid-cols-[1fr_260px]">
          <div className="p-8 lg:p-10">
            <p className="text-[10px] font-semibold tracking-[0.24em] text-[#3f9e74]">INSIGHT OF THE WEEK</p>
            <h2 className="mt-5 font-heading text-4xl leading-tight text-[#edf1f2]">Bank besar masih murah secara historis.</h2>
            <p className="mt-5 text-sm leading-8 text-[#9ba3a6]/75">
              Cost of fund turun, NIM stabil, dan kredit mulai tumbuh lagi. Tapi pasar masih menghukum karena kekhawatiran kualitas aset.
            </p>
            <a href="/articles" className="mt-8 inline-block text-[11px] font-semibold tracking-[0.18em] text-[#3f9e74]">BACA ANALISIS LENGKAP →</a>
            <div className="mt-8 flex flex-wrap gap-2">
              {["BANKING", "VALUATION", "MACRO", "IHSG"].map((t) => <span key={t} className="border border-[rgba(63,158,116,0.28)] px-3 py-1 text-[10px] tracking-[0.16em] text-[#9ba3a6]/60">{t}</span>)}
            </div>
          </div>
          <div className="hidden border-l border-[rgba(63,158,116,0.28)] bg-[#0a0b0b] p-6 md:block"><BankSketch /></div>
        </div>

        <div className="border border-[rgba(63,158,116,0.28)] bg-[#121414] p-7">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="font-heading text-3xl text-[#edf1f2]">SEKTOR PASAR</h2>
            <a href="/playbook/ihsg" className="text-[10px] font-semibold tracking-[0.18em] text-[#3f9e74]">LIHAT DETAIL →</a>
          </div>
          {sectors.length === 0 ? (
            <div className="text-xs text-[#9ba3a6]/60">Data sektor belum tersedia. Coba segarkan untuk memuat data terbaru.</div>
          ) : (
            <div className="grid grid-cols-2 gap-2 text-[10px] font-semibold tracking-[0.12em]">
              {gridSectors.map((s) => (
                <div key={s.code} className="border border-[rgba(63,158,116,0.28)] bg-[#0a0b0b] p-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[#9ba3a6]/80">{s.name}</span>
