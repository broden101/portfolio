...
export default function OrderBookPage() {
  ...
  const pairedDepth = useMemo(() => {
    ...
    // Find center index: closest price to ticker.last
    let centerIdx = 0;
    let minDist = Infinity;
    for (let i = 0; i < prices.length; i++) {
      const d = Math.abs(prices[i] - ticker.last);
    ...
    for (let k = 0; k < NUM_LEVELS; k++) {
      ...
      const offerIdx = centerIdx + 1 + k;

      const hasBid = bidIdx >= 0;
      const hasOffer = offerIdx < priceLen;
