      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between mb-10 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            {results.length > 0 && (
              <button onClick={rescreen} className="px-5 py-2.5 border border-[#8A6F3D] text-[#F4EFE6] text-xs tracking-[0.15em] uppercase font-medium hover:border-[#C6A15B] hover:text-[#C6A15B] transition-all">
                Re-screen
              </button>
            )}
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          {(["IDX100", "LQ45"] as const).map((u) => (
            <button key={u} onClick={() => changeUniverse(u)} className={`px-5 py-1.5 text-xs tracking-[0.15em] uppercase font-medium transition-all ${universe === u ? "bg-[#C6A15B]/15 text-[#C6A15B] border border-[#C6A15B]/30" : "border border-[#2C261E] text-[#B8AA96]/50 hover:text-[#B8AA96]"}`}>
              {u}
            </button>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <div className="space-y-6">
            <div className="card-luxury p-6">
              <h3 className="text-xs tracking-[0.2em] uppercase text-[#C6A15B] mb-5 font-medium">Filters</h3>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div className="relative">
                        ...
                        <div className="w-4 h-4 border border-[#2C261E] peer-checked:bg-[#C6A15B] peer-checked:border-[#C6A15B] transition-all flex items-center justify-center">
                          {f.enabled && (<svg className="w-2.5 h-2.5 text-[#0B0B0A]" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" /></svg>)}
                        </div>
                      </div>
                      <span className="text-sm text-[#F4EFE6] group-hover:text-[#C6A15B] transition-colors">{f.name}</span>
                    </label>
                      ...
                      <div className="ml-7 space-y-2 mt-2">
                        {Object.entries(f.params).map(([key, val]) => (
                          <div key={key} className="flex items-center gap-2">
                            <label className="text-xs text-[#B8AA96]/60 w-24">{key.replace(/_/g, " ")}</label>
                            <input type="number" value={val} onChange={(e) => updateParam(f.id, key, parseFloat(e.target.value) || 0)} className="w-20 bg-[#0B0B0A] border border-[#2C261E] px-2 py-1 text-xs text-[#F4EFE6] focus:border-[#C6A15B] outline-none transition-colors" step="0.5" />
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="card-luxury p-6">
              <h3 className="text-xs tracking-[0.2em] uppercase text-[#C6A15B] mb-5 font-medium">Signal</h3>
              <div className="space-y-1">
                {(["all", "BUY", "WATCH", "HOLD", "AVOID"] as const).map((mode) => (
                  <button key={mode} onClick={() => setFilterMode(mode)} className={`w-full text-left px-3 py-2 text-sm transition-colors ${filterMode === mode ? "bg-[#C6A15B]/10 text-[#C6A15B]" : "text-[#B8AA96]/50 hover:text-[#B8AA96]"}`}>
