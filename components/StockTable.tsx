...
function Badge({ action }: { action: ScreenResult["action"] }) {
  const colors: Record<string, string> = {
    BUY: "bg-[#6E6151]/15 text-[#281F17] border border-[#6E6151]/30",
    WATCH: "bg-blue-50 text-blue-700 border border-blue-200",
    HOLD: "bg-[#A39276]/10 text-[#726556] border border-[#A39276]/20",
...
function VolRatio({ vol, avg }: { vol: number; avg: number }) {
  if (!avg || avg <= 0) return <span className="text-[#A39276]">—</span>;
  const ratio = vol / avg;
  const icon = ratio >= 1.5 ? "🔥" : ratio >= 1 ? "✅" : ratio >= 0.5 ? "⚠️" : "💤";
  return <span className="text-[#726556]">{icon} {ratio.toFixed(1)}×</span>;
}

export function StockTable({ results, compact }: Props) {
  if (results.length === 0) {
    return <div className="text-center text-[#A39276] py-12 text-sm">Belum ada hasil. Jalankan screener untuk scan saham.</div>;
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-[#A39276]/20 text-left text-xs tracking-[0.15em] uppercase text-[#726556]">
        ...
        <tbody>
          {results.map((r) => (
            <tr key={r.name} className="border-b border-[#A39276]/10 hover:bg-[#A39276]/[0.05] transition-colors">
              <td className="py-3 pr-4">
                <span className="font-bold text-[#6E6151]">{r.name}</span>
                <div className="text-xs text-[#A39276] truncate max-w-[120px]">{r.desc}</div>
              </td>
              <td className="py-3 pr-4">
                <Badge action={r.action} />
                <div className="text-xs text-[#A39276] mt-1">{r.status}</div>
              </td>
              <td className="py-3 pr-4 text-right font-mono font-medium text-[#281F17]">
              ...
              <td className="py-3 pr-4 text-right text-[#726556]">
                {r.mcap ? `${(r.mcap / 1e12).toFixed(0)}T` : "—"}
              </td>
              {!compact && (
                <>
                  <td className="py-3 pr-4 text-right"><VolRatio vol={r.volume || 0} avg={r.avg_vol_10d || 0} /></td>
                  <td className="py-3 pr-4 text-right text-[#726556] font-mono">{(r.vwap || 0).toLocaleString("id-ID")}</td>
                  <td className="py-3 pr-4 text-right">
                  ...
                  <td className="py-3 pr-4 text-right">
                    <div className="flex items-center justify-end gap-0.5">
                      {[...Array(10)].map((_, i) => (
                        <div key={i} className={`w-1.5 h-3 rounded-sm ${i < r.trend_score ? "bg-[#6E6151]" : "bg-[#A39276]/20"}`} />
                      ))}
                    </div>
                  </td>
                </>
