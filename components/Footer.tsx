...
export default function Footer() {
  return (
    <footer className="border-t border-[rgba(63,158,116,0.28)] bg-[#0a0b0b]">
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 lg:grid-cols-[1.2fr_1fr_1fr_1fr] lg:px-12">
        <div>
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center border border-[#3f9e74]/40">
              <span className="font-heading text-xs font-semibold text-[#3f9e74]">R</span>
            </div>
            <span className="font-heading text-sm tracking-[0.24em] text-[#edf1f2]">RAGA PLAYBOOK</span>
          </div>
          <p className="mt-5 max-w-sm text-sm leading-7 text-[#9ba3a6]/60">
            Catatan riset pribadi tentang pasar modal Indonesia. Fokus pada valuasi, arus, dan narasi yang layak diikuti.
          </p>
          <div className="mt-6 flex gap-5 text-[10px] font-semibold tracking-[0.16em] text-[#9ba3a6]/50">
            {[
              ...
              <a key={l.label} href={l.href} aria-label={l.label} className="flex h-8 w-8 items-center justify-center border border-[rgba(63,158,116,0.28)] text-[#9ba3a6]/60 transition hover:border-[#3f9e74] hover:text-[#3f9e74]">
        ...
        {Object.entries(links).map(([title, items]) => (
          <div key={title}>
            <p className="text-[10px] font-semibold tracking-[0.24em] text-[#9ba3a6]/50">{title}</p>
            <ul className="mt-5 space-y-4 text-sm text-[#9ba3a6]/80">
              {items.map((item) => (
                <li key={item.label}>
                  <a href={item.href} className="transition hover:text-[#3f9e74]">{item.label}</a>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="border-t border-[rgba(63,158,116,0.28)]">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 text-[10px] tracking-[0.14em] text-[#9ba3a6]/30 lg:px-12">
