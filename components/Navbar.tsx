...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-[#DDBFA2]/95 backdrop-blur-md border-b border-[#A39276]/20 shadow-sm" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="text-[#726556] text-xs tracking-[0.2em] uppercase font-medium hover:text-[#6E6151] transition-colors duration-300">
                {link.label}
              </a>
            ))}
          </div>

          <button className="md:hidden text-[#726556] hover:text-[#6E6151]" onClick={() => setMenuOpen(!menuOpen)}>
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#DDBFA2]/98 backdrop-blur-md border-b border-[#A39276]/20">
